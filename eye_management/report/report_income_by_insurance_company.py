...
class ReportIncomeByInsurance(models.AbstractModel):
    _name = 'report.eye_management.income_by_insurance'
    ...
    def get_income_insurance_company(self, start_date, end_date, insurance_company):
        ...
        history_ids = self.env['account.move'].search([('invoice_date', '>=', start_date), ('invoice_date', '<=', end_date), ('state', '=', 'posted'),
                ...
                if income.insurance_company.id == insurance_company[0]:
                    ...
                    if income.payment_state == 'partial':
                        if income.insurance_company.id in prod_dict:
                            ...
                            prod_dict[income.insurance_company.id] = [income.insurance_company.name, 1,
                                                                      (income.amount_total - income.amount_residual)]
                    else:
    ...
    def formatLang(self, value, digits=None, date=False, date_time=False, grouping=True, monetary=False, dp=False, currency_obj=False, lang=False):
        ...
        return super(ReportIncomeByInsurance, self).formatLang(value, digits=digits, date=date, date_time=date_time, grouping=grouping,
